...
def write_outputs(stations: list[dict[str, Any]], output_dir: Path, stem: str) -> tuple[Path, Path]:
    ...
    exported_at = datetime.now(timezone(timedelta(hours=3))).strftime("%Y-%m-%d %H:%M:%S MSK")
    rows = [{**station, "exported_at": exported_at} for station in stations]
    ...
    with json_path.open("w", encoding="utf-8") as file:
        json.dump([{field: station.get(field, "") for field in CSV_FIELDS} for station in rows], file, ensure_ascii=False, indent=2)
...
def filter_stations_with_status(stations: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        station
        for station in stations
        if str(station.get("status_code") or "").strip().lower() not in ("", "null", "none")
    ]
